...
% %********************************** %First Section  **************************************
\section[Diabetes: A Pandemic of Unprecedented Magnitude]{Diabetes: A Pandemic of Unprecedented Magnitude}  %Section - 1.1
\label{sec:int_diabetes}

%\colorbox{green}{text clean-up} \\%\st{As of 2019, approximately 463 million adults aged 20-79 were diabetic and this number is projected to rise to ~700-800 million by 2045.}  %\st{Altogether, this makes diabetes a leading cause of mortality worldwide} %These premature deaths and comorbidities %\st{due to diabetes} 
...
% %********************************** %Second Section  **************************************
\section[The Endocrine Pancreas: Morphology and Physiology]{The Endocrine Pancreas: Morphology and Physiology}  %Section - 1.2
\label{sec:int_endopanc}

%\colorbox{pink}{incomplete figure} \colorbox{green}{text clean-up}\\%\st{. The pancreas exerts its effects} %\st{with a central role} %\st{vital organ originating from the endoderm} %The pancreas is the only organ with \st{functions both, as an} exocrine and endocrine \st{gland} functions.  \st{The exocrine} %centro-acinar %The islets are named after Paul Langerhans, who in 1869, through exhaustive histological studies discovered that the pancreas was a heterogeneous organ comprised of different structures. %along with the centro-acinar cells, %\st{ductal system} %\st{glucose homeostasis} 
...
% ********************************** % 1.3.3  **************************************
\subsection[Insulin Biosynthesis]{Insulin Biosynthesis} %Section - 1.3.3
\label{sec:insbio}
...
% ********************************** % 1.2.7  **************************************
\subsection[Insulin Action and Clearance]{Insulin Action and Clearance} %Section - 1.2.7
\label{sec:insact}
%\colorbox{pink}{missing figure} \colorbox{green}{text clean-up} \\
...
% ********************************** % 1.2.7  **************************************
\subsection[Heterogeneity of $\beta$-cells]{Heterogeneity of $\beta$-cells} %Section - 1.2.7
\label{sec:beta_het}
...
%\hl{FLTP, CD9 & Electrical coupling studies}%(see \hyperref[sec:sc_tpi]{TI methods}) 
...
%\clearpage %In addition to increased physiological demand, perturbation of \glslink{er}{ER} homeostasis can be brought about by other pathological conditions such as reactive oxygen species, infections and inflammation \textbf{\cite{fonseca_endoplasmic_2011}}.
...
The Immune Connection to \gls{t2d}}  %Section - 1.4
...
% ***************************************************************
%************************ %Fifth Section %****************************************************************
\section[Single-cell \glsentryshort{rna} Sequencing: How to Study Individual Cells?]{Single-cell \glsentryshort{rna} Sequencing:\\How to Study Individual Cells?}  %Section - 1.5
...
\par With, next-generation sequencing, the costs plummeted and ushered in a new-era of massively parallel, high-throughput sequencing capabilities for genome as well as transcriptome \textbf{\cite{voelkerding_next-generation_2009}}. Soon, after its implementation, bulk \glsentryshort{rna}-sequencing became a very popular technique for gene expression profiling due to its high sensitivity, accuracy and dynamic range. Bulk \glsentryshort{rna}-sequencing has been most often used for analyzing \glsentrylong{dge}, and in addition to that, broader applications include studying splicing events and regulation of gene expression by non-coding and enhancer \glsentryshort{rna}s \textbf{\cite{stark_rna_2019}}. However, ultimately, bulk \glsentryshort{rna}-sequencing provides an average gene expression level across all cells in a sample, which can mask differences between individual cells making it difficult to identify subtle transcriptional differences \textbf{\cite{kalisky_brief_2018}}. It is important to note that even cells within the same cell type can differ in terms of their gene expression patterns, leading to phenotypic changes. Another limitation of bulk \glsentryshort{rna}-sequencing is the requirement of large amounts of starting material which can not 

 %The average expression profile from bulk \gls{rnaseq} can obscure the true signal from rare cell populations that might be driving tumorigenesis or therapeutic resistance. 
...
\subsection[A typical \glsentryshort{scr} experiment]{A typical \glsentryshort{scr} experiment}
...
\begin{figure}[H]
    ...
    \caption[A typical \glsentryshort{scr} experiment]{\textbf{Workflow of a typical \gls{scr} experimental protocol.} The schematic illustrates an overview of steps involved in a \gls{scr} experiment to generate gene expression data. Abbreviations: LCM, laser capture microdissection; \glsentryshort{facs}, \glsentrylong{facs}. \textit{This figure is adapted from Wikipedia. Created with \href{https://www.biorender.com/}{Biorender.com}}.}
...
\subsection[10x Genomics: A droplet-based approach to \glsentryshort{scr}]{10x Genomics: A droplet-based approach to \glsentryshort{scr}}
\label{sec:scrna_10x}
...
high \glsentryshort{rna} content by using a cutoff based on the total \gls{umi} counts of each barcode. Following this, the \glsentryshort{rna} profile of the remaining barcodes is used to determine cells with low RNA content or even empty \gls{gem}s \textbf{\cite{noauthor_running_nodate,noauthor_cell_nodate}}.
...
\par Data integration is a fundamental step in \gls{scr} analysis workflow and serves to minimize batch effects and to consolidate datasets from different conditions or experiments. 
...
\par Hence, cluster annotation is a crucial step for obtaining biologically meaningful insights from \gls{scr} datasets and to understand the composition of the annotated clusters, their functional roles, and ultimately their potential roles in health and disease.
...
\section[Beyond Transcriptomics: Other Single-cell Modalities]{Beyond Transcriptomics: Other Single-cell Modalities}
...
\textbf{Single-cell Epigenomics: }In recent years, there has been rapid development in single-cell methods to assay epigenetic features such as \glsentryshort{dna} methylation with \gls{scbs} \textbf{\cite{smallwood_single-cell_2014}}, %, scWGBS \textbf{\cite{farlik_single-cell_2015}}, scRRBS \textbf{\cite{guo_single-cell_2013}} ), 
 histone modifications with \gls{chip} \textbf{\cite{grosselin_high-throughput_2019}}, %scCUT\&Tag \textbf{\cite{kaya-okur_cuttag_2019,wu_single-cell_2021}} ), 
...
characterization of cellular processes and states independent of the corresponding \gls{mrna} levels.%, with accumulating evidence pointing to a discordance between the two for most genes, at both, population and single-cell \textbf{\cite{ctortecka_rise_2021,petrosius_recent_2022,bennett_single-cell_2023}}. 
...
\section[\glsentryshort{imc}: Multiplexed Imaging in Biological Tissues]{\glsentrylong{imc}:\\Multiplexed Imaging in Biological Tissues}  %Section - 1.6
\label{sec:int_IMC}
