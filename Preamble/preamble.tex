...
%   \renewcommand{\headrulewidth}{1.2pt} % maintain the header rule
%   \fancyhead[LE,RO]{\thepage} % Page numbers on the left on even pages, right on odd pages
%   \fancyfoot[C]{\hyperlink{toc}{Back to Table of Contents}}
% }
...
\makeatletter
\renewcommand{\thesubsubsection}{\unskip}
\makeatother

% Redefine the subsubsection format to align with the paragraph
% \titleformat{\subsubsection}[runin] % makes the title run-in
%   {\normalfont\normalsize\bfseries} % bold and normal size
%   {} % no label
...
\titlespacing{\subsubsection}
  {-12pt} % left margin
  %{1.5ex plus 0.5ex minus .2ex} % vertical space before
  {15pt}
  {1em} % space after
...
\DeclareBibliographyDriver{misc}{%
  ...
  % Customizing the bibliography output for book entries
...
\DeclareFieldFormat{url}{%
  \mkbibacro{URL}\addcolon\space \footnotesize \url{#1}%
}

\DeclareFieldFormat{isbn}{%
  \mkbibacro{ISBN}\addcolon\space \isbn{#1}%
}

% Optional: To ensure the field formatting is applied, we may need to adjust bibmacros
...
% ******************************* Glossaries 
% \usepackage[toc, acronym, nopostdot, nonumberlist, style=super]{glossaries}
% \setlength{\glsdescwidth}{5in}
% \makeglossaries

\usepackage[toc, acronym, nopostdot, nonumberlist, style=super]{glossaries-extra}
...
% Enable entry counting for glossary terms
\GlsXtrEnableEntryCounting
 {acronym}% list of categories to use entry counting
 {10}% trigger value (only add to glossary if count > 3)

% Customize glossary formats
...
\renewcommand*{\cglsformat}[2]{%
  \ifglsused{#1}%
    {\glsentryshort{#1}#2}% if already used: short form
    {\glsentrylong{#1}#2 (\glsentryshort{#1})}% if first use: long form (short form)
}
...
\renewcommand*{\cglsplformat}[2]{%
  \ifglsused{#1}%
    {\glsentryshortpl{#1}#2}% if already used: short form
    {\glsentrylongpl{#1}#2 (\glsentryshortpl{#1})}% if first use: long form (short form)
}
